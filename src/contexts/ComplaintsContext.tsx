...
export const ComplaintsProvider: React.FC<ComplaintsProviderProps> = ({
	children,
}) => {
	const { user } = useAuth();
	...
	const createMutation = useMutation({
		mutationFn: (
			payload: Omit<Complaint, "id" | "createdAt" | "updatedAt" | "responses">
		) =>
			api.createComplaint({
				title: payload.title,
	...
	const addComplaint = (
		complaintData: Omit<
			Complaint,
			"id" | "createdAt" | "updatedAt" | "responses"
		>
	) => {
		if (!user) return;
