...
const ComplaintCard: React.FC<ComplaintCardProps> = ({
	...
	const canProvideFeedback =
		!isAdmin && complaint.status === "resolved" && !complaint.feedback;

	const isClosed =
		complaint.status === "resolved" || complaint.status === "rejected";
	const canRespond = isAdmin && !isClosed;

	// Auto-close response form if complaint becomes closed
	useEffect(() => {
		if (isClosed && showResponseForm) setShowResponseForm(false);
	}, [isClosed, showResponseForm]);
				...
				{complaint.responses.length > 0 && (
					<div className="mt-4">
						...
						>
							...
							{showResponses
								? "Hide Responses"
								: `Show Responses (${complaint.responses.length})`}
						</Button>
